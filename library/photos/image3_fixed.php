...
if (($image == "") AND ($GetRec['Rec_Rel_LangID'] > 0)) {
    ...
    $image = $GetRelLangRec['Rec_Image3'];
}
...
if (($RecMod_ID <> "") OR ($ModPageID <> "")) {
    ...
    if (($image == "") AND ($GetModRec['Rec_Rel_LangID'] > 0)) {
        $GetRelLangRec_Sel = "SELECT Rec_Image3 FROM records WHERE Rec_ID = ".$GetModRec["Rec_Rel_LangID"];
        $GetRelLangRec_Query = q($GetRelLangRec_Sel);
        $GetRelLangRec = f($GetRelLangRec_Query);
        $image = $GetRelLangRec['Rec_Image3'];
    }
...
if ((file_exists($imageCheck)) AND ($image <> "")) {
    ...
    print "'";
    if($_GET["le"]=="1") print " data-rec-id='$le_id' data-field='Rec_Image3'";
    print ">";
    ...
    if ($GetOptRec['Rec_Check1'] == 1) { // LazyLoad
        print "<img class=\"lazyload\" data-srcset=\""; echo srcsetPaths($image,"images",$path,$rootURL,$mobileMode); print "\" data-src=\"$rootURL$pathimage?v=$dateModified\" alt=\"$titleimg\" width=\"$widthImg\" height=\"$heightImg\" style=\"vertical-align:middle;\">";
    } else {
