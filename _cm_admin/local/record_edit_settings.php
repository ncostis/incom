...
<div class="topInternalTitle bgLighter <?php if($GetRec['Rec_Active']=='1') echo 'bgInactive'?>">
    <h2>
        <span class="cMedium"><i class="fas fa-pen"></i></span> <span class="cMedium"><small><?php echo $GetSection['Section_Title'];?> ></small></span> <?php echo $GetPage['Page_Name']; ?>
    </h2>
...
<div class="admGridFlexCont">
    <div class="admGridRecL editPanel bgContent <?php if(!$showMediaColumn) echo 'full'?>">
        ...
        <!--    Scrolling1  Languages-->
        <?php if ($GetList['List_Scroll1'] <> "") {
            ?>
            <?php
            $GetScr1_Sel = "SELECT * FROM lang Order by Lang_Title Asc";
            $GetScr1_Query = q($GetScr1_Sel);
            ?>
            <div class="admGrid4Cols">
                <div>
                    <div class="form-group">
                        <label class="formLabel"><?php echo $GetList['List_Scroll1']; ?> <i class="fas fa-question-circle admInfo cGreen" title="Define the Start Language of the Website"></i></label>
                        <div class="colInput">
                            <select name="Rec_Scroll1" class="formField">
                                <option value="0" <?php echo 'selected'; ?>>-- <?php print $textSelect; ?> --</option>
                                <?php while ($GetScr1 = f($GetScr1_Query)) { ?>
                                    <option value="<?php echo $GetScr1['Lang_Title']; ?>" <?php if ($GetScr1['Lang_Title'] == $GetRec['Rec_Scroll1']) echo 'selected'; ?>><?php echo $GetScr1['Lang_Desc']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
        ...
        foreach ($activeFields as $FieldName => $value) {
            ...
            if($FieldName == 'List_Scroll1') continue;
